Vue.component('topic', {
	data : function(){
		return {
			data : vue.global.pages.topic, //数据流载入
	...
	methods : {
		getData : function(){
			...
			$.ajax({
				url : vue.global.config.baseUrl + '/api/m/' + that.data.config.pathName + '/get?page=' + scope.datas.pageNow
					+ '&item_per_page=' + scope.datas.itemPerPage + '&topic_group_id=' + that.data.conditions.topic_group.topic_group_id,
				success : function(res){
					...
					for(var i=0;i<res.data.rows.length;i++){
						var c = JSON.parse(res.data.rows[i].content);
						var optionArray = [];
						for(var option in c.options){
							optionArray.push({
								option : option,
								content : c.options[option].content
							})
						}
						res.data.rows[i].content = optionArray
					}
		...
		init : function(){
			if(!this.data.conditions.topic_group){
				this.goBack();
				return ;
			}
			this.getData();
		...
		*/
		...
		goBack : function(){
			var scope = vue.global.pages[this.data.config.name];
			scope.conditions.topic_group = undefined;
			location.hash = 'topicgroup';
		},
	},
	mounted : function(){
		this.init();
...
<v-container>
	<v-layout row wrap>
		<v-flex xs5>
			<v-btn color="white"
			@click="goBack()">
				返回 , 当前：{{data.conditions.topic_group == undefined ? '' : data.conditions.topic_group.title}}
			</v-btn>
		</v-flex>
			...
			<v-data-table
				...
				<template slot="items" slot-scope="props">
					<td style="padding : 20px 20px 20px 20px">
						题目：{{props.item.title}}
						<br />
						<div v-for="option in props.item.content">
							{{option['option']}} : {{option['content']}}
						</div>
						<br />
						答案：{{props.item.answer}}
						<br />
						详解：{{props.item.explain}}
					</td>
				</template>
			</v-data-table>
		</v-flex>
